...
public class SecuredSharedPreference implements PreferenceProvider {

    private PreferenceProvider preferenceProvider;

    public SecuredSharedPreference(Context context, boolean enableEncryption, String sharedPreferenceName) {
        boolean isApiLevelAboveM = Build.VERSION.SDK_INT >= Build.VERSION_CODES.M;
        if (enableEncryption) {
            if (isApiLevelAboveM) {
                preferenceProvider = new AESSharedPreferences(context,sharedPreferenceName);
            } else {
                preferenceProvider = new AESSharedPreferencesLegacy(context,sharedPreferenceName);
            }
        } else {
            preferenceProvider = new SharedPreferences(context,sharedPreferenceName);
        }
    }
